...
import CheckMark from './done-icon.svg'
...
const Miners = () => {
  ...
  const [, setCluster] = useState<Cluster>('devnet');
  ...
  const createTransferTransaction = async () => {
    if (provider === undefined || provider.publicKey === null) return;
        ...
        workersHealth.map((worker) => (
          ...
          >
            ...
            <div className="space-y-2 divide-y divide-dotted divide-zinc-600 leading-10">
              ...
              <div className="grid grid-cols-2">
                <div>Total Hours</div>
                <div className="text-right">{worker.hours_idle + worker.hours_hired} hrs</div>
              </div>
